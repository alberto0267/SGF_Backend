...
export class RechazarAusencia {
  ...
  async execute(input: Input): Promise<void> {
    ...
    ausencia.rechazar(input.ownerId, input.comentario);
    await this.repo.guardar(ausencia);

    await this.notificador.notificar(ausencia.empleadoId, 'Ausencia rechazada', 'Tu ausencia ha sido rechazada.');
  }
}
